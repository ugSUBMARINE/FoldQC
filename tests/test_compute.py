...
def _token(idx: int, *, chain_id: str = "A") -> TokenInfo:
    return TokenInfo(idx, chain_id, idx + 1, "ALA", False, None)
...
def test_pae_dispatch_and_reference_requirements() -> None:
    ...
    np.testing.assert_allclose(
        compute.compute_metric("pae_row_mean", data, EMPTY_TOKEN_MAP),
        np.array([1.0, 4.0, 7.0], dtype=np.float32),
    )
    np.testing.assert_allclose(
        compute.compute_metric("pae_col_mean", data, EMPTY_TOKEN_MAP),
        np.array([3.0, 4.0, 5.0], dtype=np.float32),
    )
    np.testing.assert_allclose(
        compute.compute_metric("pae_to_sel", data, EMPTY_TOKEN_MAP, ref_indices=[0, 2]),
        np.array([1.0, 4.0, 7.0], dtype=np.float32),
    )
    np.testing.assert_allclose(
        compute.compute_metric(
            "pae_col_to_sel", data, EMPTY_TOKEN_MAP, ref_indices=[0, 2]
        ),
        np.array([3.0, 4.0, 5.0], dtype=np.float32),
    )
    np.testing.assert_allclose(
        compute.compute_metric("pae_sym_sel", data, EMPTY_TOKEN_MAP, ref_indices=[1]),
        np.array([2.0, 4.0, 6.0], dtype=np.float32),
    )
    within = compute.compute_metric(
        "pae_sym_within_sel", data, EMPTY_TOKEN_MAP, ref_indices=[0, 2]
    )
    ...
    with pytest.raises(compute.MissingReferenceError):
        compute.compute_metric("pae_to_sel", data, EMPTY_TOKEN_MAP)
    with pytest.raises(compute.MissingReferenceError):
        compute.compute_metric("pae_contact", data, EMPTY_TOKEN_MAP)
    with pytest.raises(compute.MissingContactError):
        compute.compute_metric("pae_contact", data, EMPTY_TOKEN_MAP, ref_indices=[0])

    # PAE is all-zeros: every threshold > 0 puts all tokens in one cluster.
    labels = compute.compute_metric(
        "pae_domain_complete", data, EMPTY_TOKEN_MAP, cutoff=6.5
    )
...
def test_contact_probability_dispatch_and_missing_data() -> None:
    ...
    np.testing.assert_allclose(
        compute.compute_metric("contact_prob_mean", data, EMPTY_TOKEN_MAP),
        np.array([0.5, 0.6, 0.7], dtype=np.float32),
    )
    np.testing.assert_allclose(
        compute.compute_metric(
            "contact_prob_to_sel", data, EMPTY_TOKEN_MAP, ref_indices=[0, 2]
        ),
        np.array([np.nan, 0.4, np.nan], dtype=np.float32),
        equal_nan=True,
    )
    with pytest.raises(compute.MissingReferenceError):
        compute.compute_metric("contact_prob_to_sel", data, EMPTY_TOKEN_MAP)
    with pytest.raises(compute.MissingMetricDataError):
        compute.compute_metric(
            "contact_prob_to_sel",
